...
export default function TvDevice() {
  ...
  const [filteredLantais, setFilteredLantais] = useState([]); // Tambah state untuk filter lantai
  ...
  const loadAll = async () => {
    try {
      ...
      console.log("Devices:", d.data); // Debug
      console.log("Gedungs:", g.data); // Debug
      console.log("Lantais:", l.data); // Debug
      
      setDevices(d.data.data || []);
      setGedungs(g.data.data || []);
      setLantais(l.data.data || []);
      setFilteredLantais(l.data.data || []); // Set initial filtered lantais
    } catch (error) {
  ...
  const handleGedungChange = (e) => {
    ...
    setForm({ ...form, id_gedung: selectedGedungId, id_lantai: "" }); // Reset lantai saat gedung berubah
    
    if (selectedGedungId) {
      const filtered = lantais.filter(l => l.id_gedung == selectedGedungId);
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      setEditId(null);
      setFilteredLantais(lantais); // Reset filter
      await loadAll();
    } catch (err) {
  ...
  const edit = (d) => {
    ...
    // Filter lantai sesuai gedung yang dipilih saat edit
    if (d.id_gedung) {
      const filtered = lantais.filter(l => l.id_gedung == d.id_gedung);
      setFilteredLantais(filtered);
    }
  };
  ...
  return (
    <div className="max-w-6xl mx-auto">
      ...
      {msg.text && (
        <div className={`mb-4 p-3 rounded-xl border ${msg.type === "success" ? "bg-green-600/20 border-green-400/30 text-green-200" : "bg-red-600/20 border-red-400/30 text-red-200"}`}>
          {msg.text}
        </div>
      )}
      ...
      <div className="bg-gray-800/50 border border-gray-700/30 rounded-2xl p-6 mb-8">
        <h2 className="text-xl text-gray-200 font-semibold mb-4">
          {editId ? "Edit Device" : "Tambah Device"}
        </h2>
        <form onSubmit={submit} className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <input
            className="p-3 bg-gray-700/50 border border-gray-600/30 rounded-xl text-white"
            placeholder="Nama Device (mis. TV Lantai 2 Gedung A)"
          ...
          <select
            ...
            required
            disabled={!form.id_gedung} // Disable jika belum pilih gedung
          >
          ...
          <div className="col-span-1 md:col-span-3 flex gap-2">
            {editId ? (
              <>
                <button type="button" onClick={batal} className="px-4 py-2 bg-red-600 text-white rounded-xl flex items-center gap-2">
                  <X size={18}/> Batal
                </button>
              ...
              {devices.length ? devices.map(d => (
                <tr key={d.id_device} className="border-b border-gray-700/30 hover:bg-gray-700/30">
                  ...
                  <td className="p-3 text-center">
                    <button onClick={() => edit(d)} className="px-3 py-1 mr-2 bg-yellow-500/20 text-yellow-300 rounded-lg border border-yellow-400/30 hover:bg-yellow-500/30">
